...
set tags=/home/google/workspace/tags	"ctags -R *生成 tags文件所在的路径
set autochdir

set runtimepath+= /home/google/.vim/bundle/YouCompleteMe    " 生成 YouCompleteMe文件夹所在的路径
...
let g:ycm_min_num_of_chars_for_completion=2                 " 从第 2个键入字符就开始罗列匹配项
...
" tabline中当前 buffer两端的分隔字符
let g:airline#extensions#tabline#left_sep = ' '

" tabline中未激活 buffer两端的分隔字符
...
func SetTitle() 
	...
	if &filetype == 'sh' 
		...
		if &filetype == 'cpp'		
			...
			" " 允许 backspace和光标键跨越行边界
			set whichwrap+=<,>,h,lset linespace=0

			" " 增强模式中的命令行自动完成操作
			set wildmenu

			" 可以在 buffer的任何地方使用鼠标（类似office中在工作区双击鼠标定位）
			set mouse=a ""
			...
			" 让 TagList始终解释文件中的 tag,不管 TagList窗口有没有打开  
			let Tlist_Process_File_Always=1  

			"设置 taglist打开关闭的快捷键 F4
